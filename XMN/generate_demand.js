...
areas.features.forEach((area) => {
    ...
    Object.keys(specialDemand).forEach((key) => {
        if(area.properties[key]) {
            point[key] = area.properties[key];
            if(key.startsWith("AIR_")) {
                point.id = key;
                point.jobs += specialDemand[key];
                point.remainingJobs += specialDemand[key];
            } else {
                point.id = key + "_" + ++uniCounter;
            }
        }
    });
...
const SURPRISE_LONG_COMMUTE_RATE = 0.06; // 8% chance of surprise long commute
            ...
            const jobOptions = points
                ...
                .map((j) => {
                    ...
                    if (rng() < SURPRISE_LONG_COMMUTE_RATE && distance > 2*MEDIAN_COMMUTE_KM) {
                        score += GRAVITY_WEIGHT * gravityComponent * 3;
                    }
