...
const rng = seedrandom("CKG2026");

const areas = JSON.parse(fs.readFileSync(path.join(import.meta.dirname, "grid.geojson"), "utf-8"));

const specialDemand = JSON.parse(fs.readFileSync(path.join(import.meta.dirname, "special_demand.json"), "utf-8"));
...
let specialDemandCounter = {};

Object.keys(specialDemand).forEach((key) => {
    specialDemandCounter[key] = 0;
});

areas.features.forEach((area) => {
    Object.keys(specialDemand).forEach((key) => {
        if(area.properties[key]) {
            specialDemandCounter[key] += 1
        }
    });
})

console.log(specialDemandCounter);
...
areas.features.forEach((area) => {
    ...
    if(area.properties.airport_ckg_t2) {
        point.id = `AIR_CKG_T2`;
        point.jobs += specialDemand.airport_ckg_t2;
        point.remainingJobs += specialDemand.airport_ckg_t2;
    }
    else if(area.properties.airport_ckg_t3a) {
        point.id = `AIR_CKG_T3A`;
        point.jobs += specialDemand.airport_ckg_t3a;
        point.remainingJobs += specialDemand.airport_ckg_t3a;
    } else if(area.properties.airport_ckg_t3b) {
        point.id = `AIR_CKG_T3B`;
        point.jobs += specialDemand.airport_ckg_t3b;
        point.remainingJobs += specialDemand.airport_ckg_t3b;
    } else if(area.properties.rail_chongqing_north) {
        point.id = `AIR_Chongqing North_T1`;
        point.jobs += specialDemand.rail_chongqing_north;
        point.remainingJobs += specialDemand.rail_chongqing_north;
    } else if(area.properties.rail_chongqing_west) {
        point.id = `AIR_Chongqing West_T1`;
        point.jobs += specialDemand.rail_chongqing_west;
        point.remainingJobs += specialDemand.rail_chongqing_west;
    } else if(area.properties.rail_chongqing_east) {
        point.id = `AIR_Chongqing East_T1`;
        point.jobs += specialDemand.rail_chongqing_east;
        point.remainingJobs += specialDemand.rail_chongqing_east;
    } else {
        for(let key of Object.keys(specialDemand)) {
            if(key.startsWith("airport_") || key.startsWith("rail_") || key.startsWith("air_")) {
                continue;
            }
            if(area.properties[key]) {
                point.id = `UNI_${uniCounter++}`;
                point.jobs += Math.round(specialDemand[key] / specialDemandCounter[key]);
                point.remainingJobs += Math.round(specialDemand[key] / specialDemandCounter[key]);
                break;
            }
        }
    }
            ...
            if(USE_OSRM) {
                try {
                    ...
                    if(data.code === "Ok" && data.routes.length > 0) {
                        ...
                        drivingSeconds = (drivingMeters / (nDRIVING_SPEED_KPH * 1000 / 3600));
                    }
                } catch (error) {
                    console.warn(`OSRM routing failed for ${point.id} to ${selected.j.id}, falling back to straight-line distance.`);
                    drivingMeters = turf.distance(turf.point(point.location), turf.point(selected.j.location), { units: "kilometers" }) * 1000;
